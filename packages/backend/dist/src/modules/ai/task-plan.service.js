...
exports.TaskPlanService = void 0;
const common_1 = require("@nestjs/common");
const config_1 = require("@nestjs/config");
const prisma_service_1 = require("../../prisma/prisma.service");
const calendar_service_1 = require("../calendar/calendar.service");
let TaskPlanService = class TaskPlanService {
    prisma;
    configService;
    calendarService;
    constructor(prisma, configService, calendarService) {
        this.prisma = prisma;
        this.configService = configService;
        this.calendarService = calendarService;
    }
    async generateTaskPlan(userId, goalId) {
        ...
        const totalDays = Math.ceil((goal.deadline.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        ...
        await this.prisma.taskPlan.create({
            data: {
                ...
                conflicts: taskPlan.conflicts,
            },
        });
        return taskPlan;
    }
    detectOverlaps(subtasks, existingEvents) {
        const overlaps = [];
        for (const task of subtasks) {
            ...
            for (const event of existingEvents) {
                ...
                if (taskStart < eventEnd && taskEnd > eventStart) {
                    ...
                    overlaps.push(`Task "${task.title}" (${taskStartStr}) si sovrappone con evento "${event.title}" (${eventStartStr})`);
                }
            }
        }
        return overlaps;
    }
    buildPrompt(goal, existingEvents, totalDays) {
        const now = new Date();
...
`;
    ...
    async getTaskPlan(userId, goalId) {
        return this.prisma.taskPlan.findFirst({
            where: { userId, goalId },
            orderBy: { createdAt: 'desc' },
        });
    }
    async getTaskPlanById(userId, planId) {
        return this.prisma.taskPlan.findFirst({
            where: { userId, id: planId },
        });
    }
    async updateTaskPlanStatus(userId, planId, status) {
        return this.prisma.taskPlan.updateMany({
            where: { id: planId, userId },
            data: { status },
        });
    }
    async scheduleTasksToCalendar(userId, planId) {
    ...
    async removeCalendarEventFromPlan(userId, planId, taskIndex) {
        const taskPlan = await this.prisma.taskPlan.findFirst({
            where: { id: planId, userId },
        });
        if (!taskPlan) {
        ...
        const calendarEvents = taskPlan.calendarEvents || {};
    ...
    async clearCalendarEvents(userId, planId) {
        const taskPlan = await this.prisma.taskPlan.findFirst({
            where: { id: planId, userId },
        });
        if (!taskPlan) {
    ...
    async deleteTaskPlan(userId, planId) {
        const taskPlan = await this.prisma.taskPlan.findFirst({
            where: { id: planId, userId },
        });
        if (!taskPlan) {
...
exports.TaskPlanService = TaskPlanService;
exports.TaskPlanService = TaskPlanService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        config_1.ConfigService,
        calendar_service_1.CalendarService])
], TaskPlanService);
//# sourceMappingURL=task-plan.service.js.map